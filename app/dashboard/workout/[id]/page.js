...
export default function WorkoutExecutionPage() {
  ...
  async function searchSwapExercises(q) {
    setSwapSearch(q);
    if (q.length < 2) { setSwapResults([]); return; }
    try {
      const currentEx = exercises[showSwapModal]?.mf_exercises;
      const category = currentEx?.category || '';
      const res = await fetch(`/api/exercises?search=${encodeURIComponent(q)}&category=${category}`);
      if (res.ok) {
        const data = await res.json();
        setSwapResults((data.exercises || []).filter(e => e.id !== currentEx?.id));
      }
    } catch (err) { console.error(err); }
  }

  function swapExercise(exIdx, newExercise) {
    setWorkout(prev => {
      const updated = { ...prev };
      const programExs = [...(updated.mf_program_days?.mf_program_exercises || [])];
      programExs[exIdx] = { ...programExs[exIdx], mf_exercises: newExercise, swapped_from: programExs[exIdx].mf_exercises?.name };
      updated.mf_program_days = { ...updated.mf_program_days, mf_program_exercises: programExs };
      return updated;
    });
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-44">
                ...
                <div className="px-4 pb-4 space-y-3 border-t border-gray-100 pt-3">
                  ...
                  <div className="mt-2">
                    <input
                      ...
                      className="w-full text-xs px-3 py-2 border border-gray-200 rounded-lg focus:ring-1 focus:ring-green-500 focus:border-green-500"
                      placeholder="Add exercise notes (e.g., shoulder felt tight)..."
                    />
                  </div>

                  {/* Swapped indicator */}
                  {pe.swapped_from && (
                    <div className="mt-1 text-[10px] text-amber-600 bg-amber-50 px-2 py-1 rounded">
                      Swapped from: {pe.swapped_from}
                    </div>
                  )}

                  {/* Add/Remove/Swap/Skip */}
                  <div className="flex items-center justify-between mt-2">
                    <div className="flex gap-2">
                      ...
                      <button onClick={() => setShowSwapModal(i)} className="text-xs text-blue-600 font-medium hover:text-blue-700 flex items-center gap-0.5">
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" strokeWidth={2} stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" d="M7.5 21L3 16.5m0 0L7.5 12M3 16.5h13.5m0-13.5L21 7.5m0 0L16.5 12M21 7.5H7.5" /></svg>
                        Swap
                      </button>
                    </div>
      ...
      {/* Bottom bar */}
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t p-4 space-y-2 z-30">
        {/* Workout Notes */}
        <input
          type="text"
          value={workoutNotes}
          onChange={e => setWorkoutNotes(e.target.value)}
          className="w-full text-xs px-3 py-2 border border-gray-200 rounded-lg focus:ring-1 focus:ring-green-500"
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-end sm:items-center justify-center">
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full max-w-md max-h-[70vh] flex flex-col">
            <div className="p-4 border-b flex items-center justify-between">
              <div>
                <h3 className="font-bold text-gray-900">Swap Exercise</h3>
                <p className="text-xs text-gray-500 mt-0.5">Find an alternative for {exercises[showSwapModal]?.mf_exercises?.name}</p>
              </div>
              <button onClick={() => { setShowSwapModal(null); setSwapSearch(''); setSwapResults([]); }}
            ...
            <div className="p-4 border-b">
              <input type="text" value={swapSearch} onChange={e => searchSwapExercises(e.target.value)}
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {swapResults.length === 0 && swapSearch.length >= 2 && (
                <p className="text-center text-gray-400 text-sm py-8">No matching exercises found</p>
              )}
              {swapResults.length === 0 && swapSearch.length < 2 && (
              ...
              {swapResults.map(ex => (
                <button key={ex.id} onClick={() => swapExercise(showSwapModal, ex)}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-gray-500">{ex.category} · {ex.muscle_group || ex.type}</div>
                  </div>
