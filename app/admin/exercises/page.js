...
export default function AdminExercises() {
  ...
  function addToBuilder(exercise) {
    ...
    setBuilderDays(prev => {
      ...
      day.exercises = [...day.exercises, {
        ...
        sets: exercise.default_sets || 3, reps: exercise.default_reps || 10,
        duration_seconds: exercise.default_duration_sec || 30,
        rest_seconds: exercise.default_rest_sec || 60, is_timed: exercise.is_timed || false,
      }];
  ...
  async function saveProgram() {
    ...
    try {
      const res = await fetch('/api/programs', {
        ...
        body: JSON.stringify({
          coach_id: profile.id, name: builderName, duration_weeks: 4, difficulty: 'medium',
          days: builderDays.map((day, i) => ({
            name: day.name, day_of_week: i, sort_order: i,
            exercises: day.exercises.map((ex, j) => ({
              exercise_id: ex.id, sets: ex.sets, reps: ex.reps,
              duration_seconds: ex.duration_seconds, rest_seconds: ex.rest_seconds, order_index: j,
            })),
          })),
        }),
      });
      if (res.ok) {
        ...
        setTimeout(() => {
          setShowBuilder(false); setBuilderName('');
          setBuilderDays([{ name: 'Day 1', exercises: [] }]);
          setActiveDay(0); setBuilderSaved(false);
        }, 1500);
        loadPrograms();
      }
  ...
  const loadExercises = useCallback(async () => {
    ...
    setLoading(false);
  }, [category, difficulty, exerciseType, search, page]);

  useEffect(() => { loadExercises(); }, [loadExercises]);

  // Reset page when filters change
  useEffect(() => { setPage(1); }, [category, difficulty, exerciseType, search]);

  // ---- Load programs ----
  async function loadPrograms() {
    setProgramsLoading(true);
    const user = JSON.parse(localStorage.getItem('mf_user') || localStorage.getItem('mf_profile') || '{}');
    try {
      ...
      setTeams(rosterRes.teams || []);
      setAllPlayers(rosterRes.all_players || []);
    } catch (err) { console.error(err); }
    setProgramsLoading(false);
  }

  useEffect(() => {
    if (activeTab === 'programs' && programs.length === 0) loadPrograms();
  }, [activeTab]);
  ...
  async function handleSave(e) {
    ...
    const body = {
      ...
      video_url: form.video_url || null, image_url: form.image_url || null,
    };
    if (editingId) {
    ...
    setShowAdd(false); setEditingId(null); resetForm(); loadExercises();
  }
  ...
  function openAssign(prog) {
    setShowAssign(prog); setSelectedTargets([]); setAssignSearch('');
    setAssignResult(null); setDuplicateWarning(null);
    setStartDate(new Date().toISOString().split('T')[0]); setSendVia('');
  }

  function togglePlayer(playerId) {
    setSelectedTargets(prev => prev.includes(playerId) ? prev.filter(id => id !== playerId) : [...prev, playerId]);
  }

  function selectTeam(team) {
    const teamPlayerIds = team.players.map(p => p.id);
    const allSelected = teamPlayerIds.every(id => selectedTargets.includes(id));
    if (allSelected) setSelectedTargets(prev => prev.filter(id => !teamPlayerIds.includes(id)));
    else setSelectedTargets(prev => [...new Set([...prev, ...teamPlayerIds])]);
  }
    ...
    try {
      ...
      if (checkRes.ok) {
        ...
        if (checkData.duplicates && checkData.duplicates.length > 0 && !duplicateWarning) {
          setDuplicateWarning(checkData.duplicates); setAssigning(false); return;
        }
      }
      const res = await fetch(`/api/programs/${showAssign.id}/assign`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ player_ids: selectedTargets, start_date: startDate, force: !!duplicateWarning }),
      });
      if (res.ok) {
        const data = await res.json();
        ...
        if (sendVia) {
          for (const playerId of selectedTargets) {
            try { await fetch('/api/send-url', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ player_id: playerId, type: sendVia }) }); } catch (e) {}
          }
        }
        loadPrograms();
      } else { setAssignResult({ success: false, error: 'Failed to assign' }); }
    } catch (err) { setAssignResult({ success: false, error: err.message }); }
    setAssigning(false);
  }
  ...
  const customPrograms = programs.filter(p => !p.is_template);
    ...
    <div className="p-6 max-w-[1600px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <div className="flex items-center gap-1 mt-2 bg-gray-100 rounded-lg p-0.5 w-fit">
            <button onClick={() => setActiveTab('library')}
              ...
              <span className="flex items-center gap-1.5"><IconBarbell size={15} /> Library</span>
            </button>
            <button onClick={() => setActiveTab('programs')}
              className={`px-4 py-1.5 rounded-md text-sm font-medium transition-all ${
                activeTab === 'programs' ? 'bg-white shadow-sm text-gray-900' : 'text-gray-500 hover:text-gray-700'
              }`}>
              <span className="flex items-center gap-1.5"><IconClipboardList size={15} /> Programs</span>
            </button>
          </div>
        </div>
        {activeTab === 'library' && (
        ...
        {activeTab === 'programs' && (
          <Link href="/admin/programs/new"
            className="flex items-center gap-1.5 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 font-medium text-sm transition-colors">
            <IconPlus size={16} /> Create Program
          </Link>
        )}
      </div>
      ...
      {activeTab === 'library' && (
        <>
            ...
            {CATEGORIES.map(c => {
              ...
              return (
                <button key={c} onClick={() => setCategory(c)}
                  ...
                  {c === 'all' ? `All (${totalExercises})` : `${c.replace('_', ' ').replace(/\b\w/g, l => l.toUpperCase())}${catCounts[c] ? ` (${catCounts[c]})` : ''}`}
                </button>
              );
            })}
          </div>
                  ...
                  <div key={ex.id}
                    ...
                    {hasImage ? (
                      <div className="relative aspect-square bg-gray-100 overflow-hidden">
                        <img src={ytId ? `https://img.youtube.com/vi/${ytId}/mqdefault.jpg` : ex.image_url}
                          alt="" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" loading="lazy" />
                        {ytId && (
                        ...
                        <div className={`absolute top-2 left-2 flex items-center gap-1 px-2 py-1 rounded-md text-[10px] font-semibold ${config.pill} backdrop-blur-sm`}>
                          <CatIcon size={12} /> {ex.category?.replace('_', ' ')}
                        </div>
                      </div>
                    ) : (
                      <div className={`h-2 ${
                    ...
                    <div className="p-3">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); addToBuilder(ex); }}
              ...
              {filtered.map(ex => {
                ...
                return (
                  <div key={ex.id} className="flex items-center gap-4 px-4 py-3 hover:bg-gray-50 cursor-pointer transition-colors"
                    ...
                    <span className={`px-2 py-0.5 rounded-md text-[10px] font-medium ${config.pill}`}>{ex.category?.replace('_', ' ')}</span>
                    <DifficultyDots difficulty={ex.difficulty} />
          ...
          {/* Pagination */}
          <Pagination page={page} totalPages={totalPages} total={totalExercises} onPageChange={setPage} />
        </>
      )}
              ...
              <div>
                ...
                {customPrograms.length === 0 ? (
                  <div className="bg-white rounded-xl p-8 border border-gray-200 text-center">
                    <IconClipboardList size={40} className="mx-auto mb-2 text-gray-300" />
          ...
          <div className="relative w-full max-w-md bg-white shadow-2xl flex flex-col h-full" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100 shrink-0">
              <div className="flex items-center justify-between mb-3">
                ...
                <button onClick={() => setShowBuilder(false)} className="text-gray-400 hover:text-gray-600"><IconX size={20} /></button>
              </div>
              <input value={builderName} onChange={e => setBuilderName(e.target.value)} placeholder="Program name..."
            ...
            <div className="flex items-center gap-1 px-4 pt-3 pb-2 border-b border-gray-100 overflow-x-auto shrink-0">
              {builderDays.map((day, i) => (
                <button key={i} onClick={() => setActiveDay(i)}
                  ...
                  }`}>{day.name} ({day.exercises.length})</button>
              ))}
              <button onClick={addDay} className="px-2 py-1.5 text-xs text-blue-600 hover:bg-blue-50 rounded-lg"><IconPlus size={14} /></button>
            </div>
            <div className="px-4 py-2 shrink-0">
              <input value={builderDays[activeDay]?.name || ''}
                onChange={e => { setBuilderDays(prev => { const updated = [...prev]; updated[activeDay] = { ...updated[activeDay], name: e.target.value }; return updated; }); }}
                className="w-full px-2 py-1 border border-gray-200 rounded text-sm focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Day name..." />
            </div>
              ...
              {builderDays[activeDay]?.exercises.length === 0 ? (
                ...
                <div key={ex.id} className="bg-gray-50 rounded-lg p-3 border border-gray-200">
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <button onClick={() => removeFromBuilder(activeDay, ex.id)} className="text-gray-400 hover:text-red-500"><IconX size={14} /></button>
                  </div>
                  <div className="grid grid-cols-3 gap-2">
                    <div><label className="text-[10px] text-gray-500">Sets</label>
                      <input type="number" value={ex.sets} min={1} onChange={e => updateBuilderExercise(activeDay, ex.id, 'sets', e.target.value)}
                        className="w-full px-2 py-1 border border-gray-200 rounded text-xs" /></div>
                    <div><label className="text-[10px] text-gray-500">{ex.is_timed ? 'Sec' : 'Reps'}</label>
                      <input type="number" value={ex.is_timed ? ex.duration_seconds : ex.reps} min={1}
                        ...
                        className="w-full px-2 py-1 border border-gray-200 rounded text-xs" /></div>
                    <div><label className="text-[10px] text-gray-500">Rest(s)</label>
                      <input type="number" value={ex.rest_seconds} min={0} onChange={e => updateBuilderExercise(activeDay, ex.id, 'rest_seconds', e.target.value)}
                        className="w-full px-2 py-1 border border-gray-200 rounded text-xs" /></div>
                  </div>
                </div>
              ))}
            ...
            <div className="p-4 border-t border-gray-100 shrink-0">
              {builderSaved ? (
                <div className="w-full py-3 bg-green-100 text-green-700 rounded-lg font-medium text-sm text-center">Program saved!</div>
              ) : (
          ...
          <form onSubmit={handleSave} className="bg-white rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-gray-50 border border-gray-200 rounded-xl p-3 space-y-2">
                <div className="text-xs font-medium text-gray-600 flex items-center gap-1"><IconPlayerPlay size={14} /> Media (optional)</div>
          ...
          <div className="bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-lg max-h-[85vh] flex flex-col" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-gray-100 shrink-0">
              <div className="flex items-center justify-between mb-1">
                ...
                <button onClick={() => setShowAssign(null)} className="text-gray-400 hover:text-gray-600"><IconX size={20} /></button>
              </div>
            ...
            <div className="p-4 space-y-3 overflow-y-auto flex-1">
              ...
              <div>
                <h3 className="text-xs font-semibold text-gray-500 uppercase mb-2">{assignSearch ? 'Search Results' : 'Individual Players'}</h3>
                <div className="space-y-0.5 max-h-60 overflow-y-auto">
                  {filteredPlayers.map(player => {
                    const isSelected = selectedTargets.includes(player.id);
                    return (
                      <button key={player.id} onClick={() => togglePlayer(player.id)}
                        className={`w-full flex items-center gap-3 px-3 py-2 rounded-lg text-left transition-colors ${isSelected ? 'bg-green-50' : 'hover:bg-gray-50'}`}>
                        <div className={`w-5 h-5 rounded border-2 flex items-center justify-center shrink-0 ${isSelected ? 'bg-green-600 border-green-600' : 'border-gray-300'}`}>
              ...
              <div>
                ...
                <div className="flex gap-2">
                  {[{ key: '', label: 'None' }, { key: 'sms', label: 'SMS' }, { key: 'email', label: 'Email' }, { key: 'both', label: 'Both' }].map(opt => (
